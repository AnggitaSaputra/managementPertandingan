    ...
    <div class="bg-sky-100 p-5 rounded shadow-lg items-center flex justify-between flex-col lg:flex-row">
        ...
        <div class="mt-3 lg:mt-0">
            @if ($joined_pertandingan == null)
                @php
                    $currentTime = now();
                    $registrationOpen = $countdown->dimulai > $currentTime;
                @endphp
        
                @if ($pembayaran_saat_ini != null && $pembayaran_saat_ini->status == '0' && $registrationOpen)
                ...
                @else
                    ...
                    @else
                        ...
                        @else
                            <a href="#" class="uppercase bg-gray-500 hover:bg-gray-600 cursor-not-allowed text-white py-2 px-3 rounded font-semibold" title="Pendaftaran telah ditutup">Daftar</a>
                        @endif
                    @endif
                @endif
            @else
