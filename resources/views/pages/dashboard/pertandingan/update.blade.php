...
<form method="POST" action="{{ route('pertandingan.update', $data->id) }}">
    ...
    <div class="mb-4">
        ...
        <input type="date" name="dimulai" id="dimulai" value="{{ old('dimulai', $data->dimulai) }}" class="mt-1 block w-full p-2 border border-gray-300 rounded-md @error('dimulai') border-red-500 @enderror">
        @error('dimulai')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="mb-4">
        ...
        <input type="date" name="diakhiri" id="diakhiri" value="{{ old('diakhiri', $data->diakhiri) }}" class="mt-1 block w-full p-2 border border-gray-300 rounded-md @error('diakhiri') border-red-500 @enderror">
        @error('diakhiri')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
