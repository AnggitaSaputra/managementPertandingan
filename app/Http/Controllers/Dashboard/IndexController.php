    ...
    public function index()
{
    ...
    $totalAtletPerTeam = null;
    $teamRegistered = null;
    $paymentStatus = null;

    if ($latestPertandingan && $team && Auth::user()->role != 'admin') {
        $totalAtletPerTeam = ListAtletInTeam::where('tim_id', $team->id)->count();
    ...
    $atletCountByKelas = ListListAtletWithKelas::with('kelas')
        ...
        ->groupBy('kelas_id')
        ->get();
    $atletCountByKelasArray = $atletCountByKelas->isEmpty() ? [] : $atletCountByKelas->mapWithKeys(function ($item) {
    ...
    $totalTimInPertandingan = ListTimInPertandingan::with('pertandingan')->select('pertandingan_id', DB::raw('count(*) as total'))
        ...
        ->get();
    $totalTimInPertandinganArray = $totalTimInPertandingan->isEmpty() ? [] : $totalTimInPertandingan->mapWithKeys(function ($item) {
    ...
    return view('pages.dashboard.index', [
        ...
        'totalTim' => Tim::count(),
        'totalAtletPerKelas' => $atletCountByKelasArray,
        'totalTimInPertandingan' => $totalTimInPertandinganArray,
        'team_id' => $team->id ?? null,
        'teamRegistered' => $teamRegistered ?? null,
